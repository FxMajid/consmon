...
export const IdCardActivationModal: React.FC<IdCardActivationModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!areaKerja.trim()) {
      setErrorMsg('Silakan pilih Area Kerja dari menu dropdown');
      return;
    }

    // 3. Validate Email
    if (!email.trim() || !email.includes('@')) {
      setErrorMsg('Silakan masukkan format email yang valid (contoh: nama@email.com)');
      return;
    }
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200 animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="font-black text-base tracking-tight">Formulir Aktivasi ID Card</h3>
                <span className="bg-amber-400 text-slate-950 font-black text-[10px] px-2 py-0.5 rounded-full uppercase">
                  Hasil Scan QR
                </span>
              </div>
              <p className="text-xs text-blue-100 mt-0.5">
                Lengkapi 3 data di bawah untuk mendapatkan QR Pengambilan Konsumsi
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-blue-50/70 border-b border-blue-100 px-5 py-3 flex items-center justify-between text-xs">
          <div className="flex items-center space-x-2 text-blue-900 font-medium">
            ...
            <span>ID Kartu Teralokasi:</span>
            <span className="font-mono font-bold bg-white px-2 py-0.5 rounded-md border border-blue-200 text-blue-800">
              {selectedCardId || 'Auto Generated'}
            </span>
          </div>
          <button
            type="button"
          ...
          >
            {showAdvancedCardPicker ? 'Sembunyikan Opsi ID' : 'Ganti ID Fisik'}
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4">
          ...
          {showAdvancedCardPicker && (
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl space-y-1.5 animate-in fade-in">
              <label className="block text-[11px] font-bold text-slate-700 uppercase tracking-wider">
                Pilih Nomor Seri ID Card Fisik (Opsional Panitia):
              </label>
              <select
              ...
              >
                {allCards.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.id} - {c.status === 'active' ? `[Terdaftar: ${c.holderName}]` : '[Siap Diaktivasi]'}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[10px] text-slate-400 font-medium">Input Teks Bebas</span>
            </div>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Contoh: Raditya Pratama / Meli Yustika"
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider">
                2. Area Kerja (Drop Down) <span className="text-rose-500">*</span>:
              </label>
              <span className="text-[10px] text-blue-600 font-semibold">Pilih Sesuai Data Lapangan</span>
            </div>
            <div className="relative">
            ...
            <div className="flex flex-wrap gap-1 mt-2">
              {['Backstage', 'Main Stage', 'KONSUMSI', 'REGISTRASI', 'BOOTH GAMES', 'Tenda Medis'].map((areaTag) => (
                <button
                  type="button"
                  key={areaTag}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider">
                3. Email Panitia / Peserta <span className="text-rose-500">*</span>:
              </label>
              <span className="text-[10px] text-slate-400 font-medium">Pengiriman QR Backup</span>
            </div>
            <div className="relative">
              ...
              <input
                ...
                placeholder="nama.anda@event-hbd.id"
                className="w-full pl-10 pr-3.5 py-2.5 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-600 font-semibold placeholder:text-slate-400 bg-slate-50/50 focus:bg-white"
                required
              />
            </div>
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3.5 text-xs text-emerald-950 flex items-start space-x-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-emerald-800 mt-0.5 leading-relaxed">
                Layar HP Anda akan langsung menampilkan <strong>QR Pengambilan Konsumsi</strong> resmi. Simpan atau screenshot QR tersebut untuk discan oleh petugas pos saat mengambil makan (Pagi, Siang &amp; Malam).
              </p>
            </div>
          </div>
          ...
          <div className="pt-2 flex items-center justify-end space-x-2 border-t border-slate-100">
            ...
            <button
              type="submit"
              className="inline-flex items-center space-x-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-bold shadow-lg shadow-blue-600/20 transition-all"
            >
              <span>Submit &amp; Dapatkan QR Pengambilan</span>
